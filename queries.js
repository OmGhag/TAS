...
const getUsers = (request, response) => {
  var {FROM_DATE,TO_DATE,FROM_TIME,TO_TIME,T_CODE} = (request.body)
    pool.query('SELECT AVAILABLE FROM CMPN_UPDATE_TABLE WHERE FROM_DATE=$1 AND TO_DATE=$2 AND FROM_TIME=$3 AND TO_TIME=$4 AND T_CODE=$5 ', [FROM_DATE,TO_DATE,FROM_TIME,TO_TIME,T_CODE], (error, results) => {
      if (error) {
        throw error
        }
